...
\ \\
\textbf{Differential von rationalen Funktionen 1} \textit{[vlg. Chapter 16.5 \ModulsOfDifferenzials]}
\begin{bsp}\comment{\label{Differential von rationalen Funktionen 1}}
...
\end{lemma}
\begin{proof}
Wähle $\alpha \in T$ mit $L[\alpha] = T$. Sei weiter f(x) das Minimalpolynom von $\alpha$. Betrachte dazu die conormale Sequenz von  $\functionfront{\pi}{L[x]}{L[x]/(f) \simeq T}$ aus \cref{Konormale Sequenz}:
\begin{center}
\begin{tikzcd}
