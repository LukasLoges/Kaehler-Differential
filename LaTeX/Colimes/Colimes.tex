...
mit $\zeta_{B_{\gamma}} = \psi^2_{\gamma(B_{\gamma})}$ für $B_{\gamma} \in \lbrace B_{\gamma} \vert \gamma \in Morph_{\mathcal{B}}\rbrace$.\\
...
\end{tikzcd}
\ \\
Wähle $f \in Hom_{\mathcal{B}}(C_1,C_2)$ als die eindeutige Funktion, für die $\zeta = \psi^1 \circ f$ gilt.\\
\ \\
Für $g$ betrachte den Morphismus $\functionfront{\zeta'}{\lbrace B_{\gamma} \vert \gamma \in Morph_{\mathcal{B}}\rbrace}{C_2}$,\\
mit $\zeta'_{B_{\gamma}} = \psi^2_{\gamma(B_{\gamma})} \circ \gamma$ für $B_{\gamma} \in \lbrace B_{\gamma} \vert \gamma \in Morph_{\mathcal{B}}\rbrace$.\\
...
\end{tikzcd}
\ \\
Wähle $g \in Hom_{\mathcal{B}}(C_1,C_2)$ als die eindeutige Funktion, für die $\zeta' = \psi^1 \circ g$ gilt.
\end{center}
Sei nun also $C \in Obj_{\mathcal{B}}$ zusammen mit $q \in Hom_{\mathcal{A}}(C_2,C)$ der Differenzkokern von $f$,$g$.\\
...
Nutze dazu zunächst, dass $(C_2,\psi^2)$ der Kolimes von $\lbrace B \vert B \in Obj_{\mathcal{B}} \rbrace \subseteq \mathcal{B}$ ist und somit dessen universelle Eigenschaft besitzt:
\begin{itemize}
\item[]
...
\end{center}
\end{itemize} 
Um die universelle Eigenschaft von $q$ nutzen zu können zeige nun, dass $q' \circ f = q' \circ g$ gilt. Sei dazu $c \in C_1$ beliebig und wähle $\gamma \in Morph_{\mathcal{B}}$ und $b \in B_{\gamma}$ mit $\psi^1_{B_{\gamma}}(b) = c$, dann gilt:
\begin{gather*}
(q' \circ f)(c)
...
= \psi'_{B_{\gamma}}(b) \\
\end{gather*}
Folglich können wir die universelle Eigenschaft von $q$ auf $q'$ anwenden und erhalten ein eindeutiges $\varphi \in Hom(C,C')$ mit $q' = q \circ \varphi$. Dieses erfüllt somit auch:
\begin{center}
$\psi \circ \varphi = \psi^2 \circ q \circ \varphi = \psi^2 \circ q' = \psi'$\\
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
Damit haben wir gezeigt, dass $\colimes \mathcal{B} = (C,\psi)$ existiert.
\end{proof}
