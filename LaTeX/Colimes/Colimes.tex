...
\end{prop}
\begin{proof} \ \\
\underline{Zu \textit{\textbf{1.}}:} Sei $\mathcal{B}$ die Unterkategorie der R-Algebren, welche $\lbrace S_i \rbrace_{i \in \Lambda}$ zusammen mit den Identitätsabbildungen enthält. Seien weiter:
\begin{itemize}
...
\ \\
\ \\
$\functionfront{\varphi}{\colimes \mathcal{B}}{\bigotimes_{i \in \Lambda} S_i} \hspace{9 em} \functionfront{\phi}{\bigotimes_{i \in \Lambda} S_i}{\colimes \mathcal{B}}$
\end{center}
Die universellen Eigenschaften liefern uns nun, dass $\varphi$ und $\phi$ zueinander Inverse sind.
\begin{center}
\begin{tikzcd}
         & \mathcal{B} \arrow[rd, "\psi"] \arrow[ld, "\psi"'] &                                                                              &  &                  & \bigoplus_i S_i \arrow[rd, "g"] \arrow[ld, "g"'] &                                                                                     \\
\colimes \mathcal{B} &                                                    & \coprod_{i \in \Lambda} S_i \arrow[ll, "\exists ! id_{\colimes \mathcal{B}} = \phi \circ \varphi "', dashed] &  & \bigotimes_{i \in \Lambda} S_i &                                                  & \bigotimes_i S_i \arrow[ll, "\exists ! id_{\bigotimes_i S_i} = \varphi \circ \phi "', dashed]
\end{tikzcd}
\end{center}
Damit haben wir Isomorphismen zwischen $\colimes \mathcal{B}$ und $\bigotimes_i S_i$ gefunden.\\
Da das Koprodukt von $\lbrace S_i \rbrace_{i \in \Lambda}$ bis auf eine eindeutige Isomorphie eindeutig bestimmt ist, definiere dies ab jetzt als $\bigotimes_{i \in \Lambda} S_i$.\\
\ \\
\underline{Zu \textit{\textbf{2.}}:} Zeige, dass $(S_2/Q,\functionfront{q}{S_2}{S_2/Q})$ die in \cref{NeuDifferenzenkokerndef} eingeführten Eigenschaften des Differenzkokern`s  besitzt.
\begin{gather*}
