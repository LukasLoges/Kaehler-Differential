...
\item[\underline{\textbf{1.}"$\Leftarrow$":}] Sei B eine p-Basis von T über k.\\
...
Betrachte dazu die COTANGENT SEQUENZ \comment{\label{*Prop 16.2}} von $K \hookrightarrow (k * K^p) \hookrightarrow T$.
\begin{center}
\begin{tikzcd}
\Tensor{T}{(k * K^p)}{\divR{(k * K^p)}{k}} \arrow[r, ""] & \divR{T}{k} \arrow[r, ""] & \divR{T}{(k * K^p)} \arrow[r, ""] & 0
\end{tikzcd}
\end{center}
\end{itemize}
\end{proof}
